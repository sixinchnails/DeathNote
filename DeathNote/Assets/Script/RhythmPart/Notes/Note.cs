...
public class Note : MonoBehaviour, IPointerDownHandler
{
    ...
    void OnEnable()
    {
        ...
        transform.rotation = Quaternion.Euler(0, 0, 0); // 회전여부 초기화
    ...
    void Update()
    {
        ...
        // 회전 애니메이션
        Quaternion startRotation = Quaternion.Euler(0, 0, 0); // 시작 회전
        Quaternion endRotation = Quaternion.Euler(0, 360, 0); // 목표 회전
        transform.rotation = Quaternion.Lerp(startRotation, endRotation, lerpValue);
